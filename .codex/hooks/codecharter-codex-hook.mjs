...
const localBin = join(
  ...
  "node_modules",
  ".bin",
  process.platform === "win32" ? "codecharter.cmd" : "codecharter",
);
const localTsx = join(
  root,
  "node_modules",
...
const localSourceCli =
  existsSync(localTsx) && existsSync(localCli)
    ? [{ command: localTsx, args: [localCli, "codex-hook"] }]
    : [];
const candidates = existsSync(localBin)
  ? [
      { command: localBin, args: ["codex-hook"] },
      ...localSourceCli,
      { command: "codecharter", args: ["codex-hook"] },
      { command: "npx", args: ["--yes", "codecharter@0.2.0", "codex-hook"] },
    ]
  : [
      ...localSourceCli,
      { command: "codecharter", args: ["codex-hook"] },
