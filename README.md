...
This repository is a Node >=22, ESM, pnpm-workspaces monorepo with a core engine
...
pnpm test                # root smoke test plus workspace test suites
pnpm build               # build the publishable dist/ package
pnpm generate            # generate .codecharter/codecharter.json for this repo
pnpm serve               # start the localhost server (serves the viewer + API)
```

The CLI is `core/bin/codemap.mts` (commands: `generate`, `resolve`, `serve`,
`dev`, `doctor`, `init`). Run it locally via
`pnpm codecharter -- <command>`.

## Tests

Run `pnpm test` for the root smoke test and the core and viewer suites. Use
