/**
 * One-time setup / provisioning (**BR-054**): write `.codecharter/config.json`,
...
class CodexHooksMerger {
  ...
  hookKeySet(hooks: CodexHook[] = []): Set<string> {
    return new Set(hooks.map((hook) => this.hookKey(hook)));
  }
...
1. \`${localBinCommand} --json resolve ...\`
2. \`${localSourceCommand} --json resolve ...\` when this is a CodeCharter source checkout
3. \`${npxCommand} --json resolve ...\`
...
2. If the binary is missing, rerun the same resolve through \`${localBinCommand}\`, \`${localSourceCommand}\`, then \`${npxCommand}\` in that order.
...
- Use \`${localBinCommand} --json resolve ...\` when the package is installed locally.
- Use \`${localSourceCommand} --json resolve ...\` in a source checkout that has not built the package bin.
- Use \`${npxCommand} --json resolve ...\` only after local runners are unavailable.
