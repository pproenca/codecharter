...
const ACTIVITY_EVENT_STRING_FIELDS = [
  "id", "agentId", "activityState", "state", "timestamp", "note",
...
function assertLocalHost(request: IncomingMessage): void {
  ...
  if (!LOCAL_HOSTNAMES.has(hostname)) throw httpError(403, "Forbidden host");
}
...
async function serveStatic(state: ServerState, response: ServerResponse, pathname: string): Promise<void> {
  ...
  if (path !== state.publicRoot && !path.startsWith(state.publicRoot + sep)) throw httpError(400, "Invalid path");
  try {
...
async function loadCodemap(state: ServerState): Promise<CodecharterCodemap> {
  ...
  if (state.codemapCache?.signature === signature) return state.codemapCache.codemap;

  let parsed: unknown;
  try {
    parsed = JSON.parse(await readFile(state.mapPath, "utf8"));
  } catch (error) {
    if (error instanceof SyntaxError) throw httpError(500, "Map file is not valid JSON; run `codecharter generate`");
    throw error;
  }
...
async function readBody(request: IncomingMessage): Promise<JsonObject> {
  ...
  for await (const chunk of request) {
    total += chunk.length;
    if (total > MAX_BODY_BYTES) throw httpError(413, "Request body too large"); // HARDENING (DoS)
...
function assertWithinRoot(root: string, candidate: string): void {
  ...
  if (full !== root && !full.startsWith(root + sep)) throw httpError(400, "Path escapes repository root");
}
