...
- `.codex/hooks/codecharter-codex-hook.mjs` is a checked-in local hook shim generated by `codecharter init`; keep implementation work in TypeScript and regenerate the shim from the initializer template.
- `public-src/` is checked by `tsc -p tsconfig.public.json`; do not add `// @ts-nocheck` or `noCheck` bridges back.
- Ongoing TypeScript cleanup should keep unsafe JSON/fetch/error boundaries behind small guard or builder functions, and keep any JavaScript exceptions explicitly generated or shim-only.
