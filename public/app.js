...
import { SOURCE_TEXT_MAX_LINES_PER_FRAME, SOURCE_TEXT_PREFETCH_LINES, activityFragmentBounds, activityFeedEvents, activityPrimaryBounds, activityStateStyle, activityTrailGroups, activityTrailPointGroups, activityTissueBox, activityVisualEncoding, activityActorKey, activityActorLabel, annotationClipboardText, buildActivityFogState, boundsCenter as modelBoundsCenter, cachedSourceRange, canvasKeyboardAction, canRenderSourceText, documentKeyboardAction, discoveryFogRevealStyle, discoveryFogVeilStyle, doubleClickMapAction, draftSelectionFromDrag, fileLabelPriority, fileVisualState, fogStateForFile, fogStateForFolder, folderDepth, folderDisplayName, folderLabelPriority, folderStyle, hashString, hashRouteFocusIntent, hitTestActivityEvents, hitTestAnnotations, hitTestTargets, interactionModeUiState, isSpaceKeyEvent, isUsableDraftSelection, isScreenBoxVisible, KEYBOARD_ZOOM_FACTOR, labelBoxesOverlap, lineHeightForFile, lineAtWorldPoint, latestActivityByAgent, mapHoverLabel, mapRouteFocusAction, mapRouteTarget, mapSearchAction, mapSearchMatch, mapSelectionPanel, mapTargetSelectionAction, normalizeActivityState, organicTrailSegments, organicRegionFolders, organicRegionPoints, organicRegionStyle, panViewForDrag, panViewByScreenDelta, reconciledSelectedTarget, screenBoundsForView, screenToWorldPoint, shouldDrawFolder, shouldDrawOrganicRegion, shouldLabelFoggedFile, shouldLabelFolder, shouldShowFogLabel, shouldShowFogSourceText, rememberSourceRange, sourceContextRequest, sourcePanelLineRangeForBox, sourcePanelState, sourceRangeCacheKey, sourceTextLayoutForBox, sortedActivityEvents, viewForBounds, viewForReadableFile, visibleLineRangeForBox, worldToScreenPoint, zoomViewAt, } from "./render-model.js";
import { boundsFromRouteParams, createAnnotationHashRoute, createMapHashRoute, createSelectionHashRoute, parseHashRoute, } from "./deep-links.js";
const canvas = requiredElement(document.querySelector("#mapCanvas"), "map canvas");
...
const mapArea = requiredElement(document.querySelector(".map-area"), "map area");
...
function updateSelectionPopover() {
    ...
    if (controls.annotationActions)
        controls.annotationActions.hidden = !selectedAnnotation || hasDraft || isEditing;
    if (controls.deleteAnnotation)
...
function drawDiscoveryVeil(rect, targetCtx) {
    ...
    const vignette = targetCtx.createRadialGradient(rect.width * 0.48, rect.height * 0.48, 0, rect.width * 0.5, rect.height * 0.5, Math.max(rect.width, rect.height) * 0.72);
...
function buildDiscoveryFogMask(rect) {
    fogMaskCtx.save();
    fogMaskCtx.setTransform((window.devicePixelRatio || 1) * FOG_MASK_SCALE, 0, 0, (window.devicePixelRatio || 1) * FOG_MASK_SCALE, 0, 0);
...
function drawFiles() {
    ...
    for (const file of state.mapFiles) {
        ...
        if (shouldShowFogSourceText(fogState, { selected })
...
function drawActivityMembranes(events, latestByAgent, { discoveryMode = false } = {}) {
    for (const event of events) {
        ...
        const selected = state.selectedTarget?.targetType === "activity" && state.selectedTarget.id === event.id;
        const encoding = activityVisualEncoding(event, { latest, selected });
        if (discoveryMode && state.view.scale > 6 && !selected)
            continue;
        if (discoveryMode && !latest && !selected)
            continue;
...
function activityTrailPoints(events) {
    ...
    for (const event of events) {
        ...
        if (bounds)
            points.push(worldToScreen(boundsCenter(bounds)));
    }
    return points;
}
...
function drawActivityTissue(box, key) {
    ...
    for (let index = 1; index <= points; index += 1) {
        ...
        ctx.lineTo(center.x + Math.cos(angle) * radiusX * wobble, center.y + Math.sin(angle) * radiusY * wobble);
    }
    ctx.closePath();
}
...
function onCanvasKeyDown(event) {
    ...
    if (!action || !CANVAS_KEYBOARD_COMMANDS.has(action.type))
        return;
    event.preventDefault();
...
function onDocumentKeyDown(event) {
    ...
    if (!action || !DOCUMENT_KEYBOARD_COMMANDS.has(action.type))
        return;
    event.preventDefault();
...
function requiredContext(context, name) {
    if (!context)
        throw new Error(`Missing ${name}`);
    return context;
}
