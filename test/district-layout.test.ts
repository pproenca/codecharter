...
import { DistrictLayoutEngine, layoutChildren } from "../src/district-layout.ts";
import type { LayoutTarget } from "../src/district-layout.js";
...
function folder(path: string, weight: number, childCount = 0): LayoutTarget {
  return {
    type: "folder",
    path,
    weight,
...
function file(path: string, weight: number): LayoutTarget {
  return {
    type: "file",
    path,
    weight,
...
function boundsByPath(children: LayoutTarget[]): Record<string, LayoutTarget["bounds"]> {
