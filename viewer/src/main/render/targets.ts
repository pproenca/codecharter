...
export function hashRouteFocusIntent(
  route: MapRoute | null | undefined,
  { hasMap = true } = {},
): HashRouteFocusIntent | null {
  if (!route || !hasMap) {
    return null;
  }
...
export function mapRouteFocusAction(
  target: TargetHit | null | undefined,
): MapRouteFocusAction | null {
  if (!target) {
    return null;
  }
...
export function mapHoverLabel(
  ...
  if (hit.targetType === "activity") {
    ...
    const actor = thread
      ? `${hit.agentId ?? "agent"} ${shortActivityId(thread)}`
      : (hit.agentId ?? "agent");
...
export function hitTestTargetLists(
  files: Iterable<MapFile>,
  folders: Iterable<MapFolder>,
  point: Point,
): TargetHit | null {
...
export function hitTestAnnotations(
  ...
  point: Point,
  { radiusX = 0, radiusY = 0 } = {},
): (NamedPlace & { targetType: "annotation" }) | null {
...
function firstSearchTarget<T extends MapTarget>(
  targets: MapTargetRecord<T>,
  query: string,
): T | null {
...
function firstGeohashTarget<T extends MapTarget>(
  targets: Iterable<T>,
  geohash: string,
  accept: (target: T) => boolean = () => true,
): T | null {
...
function bestContainingTarget<T extends MapTarget>(
  targets: Iterable<T>,
  point: Point,
  accept: (target: T) => boolean = (_target) => true,
): T | null {
